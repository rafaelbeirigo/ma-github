...
Some preparation before the command is called will be needed, so we'll leave a placeholder for it.

~<<Create remote repository>> +≡~

#+begin_src elisp :noweb-ref Create remote repository
<<Make sure we have all we need to create the remote repository>>
...
The first thing we need is the Github API URL.
...
#+begin_src elisp :noweb-ref Make sure we have all we need to create the remote repository
(let ((api-url
...
Let's create a variable for that too.
...
Now let's make sure we get a username.
